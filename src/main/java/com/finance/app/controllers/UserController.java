...
public class UserController {
    ...
    public List<UserDto> findAll(){
        return userDetailService.findAll().stream().map(userConverter::entityToDto).collect(Collectors.toList());
    }

    @GetMapping("/{id}")
    public UserDto findById(@PathVariable Long id){
        return userConverter.entityToDto(userDetailService.findById(id));
    }
    ...
    public ResponseEntity<?> createUser (@RequestBody RegistrationUserDto registrationUserDto){
        ...
        if (userDetailService.existsByUsername(registrationUserDto.getUsername())){
